...
const LINE_DATA_ROOT = "https://api-data.line.me/v2/bot";
...
export class LineClient {
  ...
  async getMessageContent(messageId) {
    const response = await fetch(`${LINE_DATA_ROOT}/message/${messageId}/content`, {
  ...
  async getSourceProfile(source) {
    ...
    try {
      ...
      if (source.type === "user") return await this.request(`/profile/${source.userId}`);
    } catch {
      return null;
    }
    return null;
